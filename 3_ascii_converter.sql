...
-- Обновляем таблицу: заменяем текстовые значения на ASCII-код первого символа
UPDATE demographics
SET
    name = ASCII(SUBSTRING(name FROM 1 FOR 1))::TEXT,
    race = ASCII(SUBSTRING(race FROM 1 FOR 1))::TEXT;
